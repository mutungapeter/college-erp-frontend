...
const Programmes = () => {
  ...
  const [selectedProgramme, setSelectedProgramme] = useState<number | null>(
    null,
  );
  ...
  const accountsPermissions = useMemo(() => {
    return permissions.find(
      (permission) => permission.module.code === 'academics_programmes',
    );
  }, [permissions]);
  ...
  const { data, isLoading, error, refetch } = useGetProgrammesQuery(
    queryParams,
    {
  ...
  const handleViewDetails = (id: number) => {
    router.push(`/dashboard/curriculum/programmes/${id}`);
  };
