...
const EditAcadmicYear = ({ refetchData, data }: Props) => {
  const [isOpen, setIsOpen] = useState(false);

  const [updateAcademicYear, { isLoading: isCreating }] =
  ...
  const onSubmit = async (formData: AcademicYearFormDataType) => {
    ...
    try {
      ...
      const msg = res?.message || 'Academic Year  updated successfully!';
      toast.success(msg);
      handleCloseModal();
      refetchData();
    } catch (error: unknown) {
...
export default EditAcadmicYear;
