...
import { CourseSessionType, ProgrammeCohortType } from "@/definitions/curiculum";
import { PAGE_SIZE } from "@/lib/constants";
import { useGetCohortsQuery } from "@/store/services/curriculum/cohortsService";
import { useDeleteCourseSessionMutation, useGetCourseSessionsQuery } from "@/store/services/curriculum/courseSessionService";
...
const CourseSessions = () => {
  ...
  const { data, isLoading, error, refetch } = useGetCourseSessionsQuery(queryParams, {
    refetchOnMountOrArgChange: true,
  });
  const { data:cohorts } = useGetCohortsQuery({}, {refetchOnMountOrArgChange: true,});
const [deleteCourseSession, {isLoading:isDeleting}] = useDeleteCourseSessionMutation();   
   console.log("cohorts", cohorts)
const CohortOptions = cohorts?.map((cohort:ProgrammeCohortType) => ({
    value: cohort.id, 
    label: cohort.name,
  })) || [];
 ...
 const handleDeleteCourseSession = async () => {
  ...
  const columns: Column<CourseSessionType>[] = [
    ...
    },
    {
      ...
      accessor: "start_time",
      cell: (item: CourseSessionType) => (
        <span className="text-sm font-normal">{CustomDate(item.start_time)}</span>
      ),
    },
    {
      header: "Period",
    ...
    <>
      <div className="bg-white w-full  p-1 shadow-md rounded-lg font-nunito">
        <div className=" p-3  flex flex-col md:flex-row md:items-center lg:items-center md:gap-0 lg:gap-0 gap-4 lg:justify-between md:justify-between">
          <h2 className="font-semibold text-black text-xl">All Courses Sessions</h2>
          <div>
