...
import { useDeleteSemesterMutation, useGetSemestersQuery } from "@/store/services/curriculum/semestersService";
...
const Semesters = () => {
  ...
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
    const [selectedSemester, setSelectedSemeseter] = useState<number | null>(null);
  ...
  const { data, isLoading, error, refetch } = useGetSemestersQuery(queryParams, {
  ...
  const columns: Column<SemesterType>[] = [
    ...
    },
    {
      ...
      cell: (item: SemesterType) => (
        <span className="text-sm font-normal">{item.academic_year}</span>
      ),
    },
    {
      header: "Start Date",
      accessor: "start_date",
      cell: (item: SemesterType) => (
        <span className="text-sm font-normal">{CustomDate(item.start_date)}</span>
      ),
    },
    {
      header: "End Date",
    ...
    <>
      <div className="bg-white w-full  p-1 shadow-md rounded-lg font-nunito">
        <div className=" p-3  flex flex-col md:flex-row md:items-center lg:items-center md:gap-0 lg:gap-0 gap-4 lg:justify-between md:justify-between">
          ...
          <div>
            <AddSemester refetchData={refetch} />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 mt-5 lg:gap-0 md:gap-0 lg:flex-row md:flex-row  md:items-center p-2 md:justify-between lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-3  lg:p-0 lg:flex-row md:flex-row md:items-center md:space-x-2 lg:items-center lg:space-x-5">
            ...
            value={SemesterStatusOptions.find(
