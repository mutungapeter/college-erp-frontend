...
import MetricCard from '../admissions/applications/CardData';
...
const Students = () => {
  ...
  const { data: metricsData } =
    useGetStudentMetricsQuery(queryParams, { refetchOnMountOrArgChange: true });
  ...
  const cardData = [
    {
      ...
      total: metricsData?.total_students ?? 0,
      icon: <PiStudentLight className="text-[#771BCC]" size={24} />,
      bgColor: 'bg-[#F3EEF6]',
    },
    {
      title: 'Active',
      total: metricsData?.Active ?? 0,
      icon: <AiOutlineCheckCircle className="text-[#22C55E]" size={24} />,
      bgColor: 'bg-[#DCFCE7]',
    },
    {
      title: 'Inactive',
      total: metricsData?.Inactive ?? 0,
      icon: <MdOutlineDoNotDisturbOn className="text-[#9CA3AF]" size={24} />,
      bgColor: 'bg-[#F3F4F6]',
    },
    {
      title: 'Graduated',
      total: metricsData?.Graduated ?? 0,
      icon: <FaUserGraduate className="text-[#2563EB]" size={24} />,
      bgColor: 'bg-[#DBEAFE]',
    },
    // {
    //   title: "Suspended",
    //   total: metricsData?.Suspended ?? 0,
    //   icon: <MdOutlineBlock className="text-[#F87171]" size={24} />,
    //   bgColor: "bg-[#FEE2E2]",
    // },
    {
      title: 'Dropped',
      total: metricsData?.Dropped ?? 0,
      icon: <RiUserUnfollowLine className="text-[#F59E0B]" size={24} />,
      bgColor: 'bg-[#FEF3C7]',
    },
  ];
  ...
  return (
    <>
      <div className="flex items-center justify-between w-full p-2 font-poppins  ">
        <div className="space-y-4">
          ...
          <p className="font-inter font-light">
            Manage students Across Departments and Programmes{' '}
          </p>
        </div>
        <div className="flex items-center space-x-2 justify-betwee">
          <div>
      ...
      <div
        className="flex flex-col gap-4 mt-6 mb-6 lg:gap-0 md:gap-0 lg:flex-row 
     md:flex-row  md:items-center p-2 md:justify-end lg:items-center lg:justify-end"
      >
        <div
          className="flex flex-col gap-3  lg:p-0 lg:flex-row md:flex-row 
     md:items-center md:space-x-2 lg:items-center lg:space-x-5"
        >
          <FilterSelect
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 mb-6 gap-6">
        {cardData.map((card) => (
          <MetricCard
            key={card.title}
            title={card.title}
