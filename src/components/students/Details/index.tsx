...
const StudentDetails = ({ student_id }: Props) => {
  ...
  const [activeTab, setActiveTab] = useState<"personal" | "programme" | "hostel" | "guardian" | "campus">("personal");
  ...
  if (isError || !data) {
    return (
      <div className="bg-red-50 text-red-700 p-4 rounded-lg mt-10 max-w-4xl mx-auto">
        <p className="font-medium">Failed to load student details. Please try again later.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto font-nunito">
      ...
      {activeTab === "personal" && <PersonalInfoTab studentDetails={studentDetails} refetchData={refetch} />}
      {activeTab === "programme" && <ProgrammeDetailsTab studentDetails={studentDetails} refetchData={refetch} />}
      {activeTab === "guardian" && <GuardianDetailsTab studentDetails={studentDetails} refetchData={refetch} />}
      {activeTab === "campus" && <CampusDetailsTab studentDetails={studentDetails} refetchData={refetch} />}
      {activeTab === "hostel" && <HostelDetailsTab studentDetails={studentDetails} />}
    </div>
  );
};
