...
const StaffDetails = ({ staff_id }: Props) => {
  const { data, isLoading, error, refetch } = useGetStaffDetailsQuery(
    staff_id,
    {
  ...
  return (
    <>
      <div className="w-full md:max-w-c-1235 mx-auto font-nunito">
        ...
        {activeTab === 'personal' && (
          <PersonalInfoTab data={staffDetails} refetchData={refetch} />
        )}
        {activeTab === 'work' && (
          <WorkDetailsTab data={staffDetails} refetchData={refetch} />
        )}
      </div>
    </>
  );
