...
const styles = StyleSheet.create({
  ...
  header: { marginBottom: 20, borderBottomWidth: 2, borderBottomColor: "#10b981", paddingBottom: 15, alignItems: "center" },
  logo: { width: 80, height: 80, marginBottom: 5 },
  schoolName: { fontSize: 16, fontWeight: "bold", marginBottom: 8, color: "#1f2937" },
  contactInfo: { fontSize: 8, color: "#6b7280" },
  documentTitle: { fontSize: 12, fontWeight: "bold", marginTop: 8, color: "#374151" },
  studentInfoSection: { marginBottom: 15, padding: 12, flexDirection: "row", justifyContent: "space-between", backgroundColor: "#f9fafb" },
  infoColumn: { width: "40%" },
  infoRow: { flexDirection: "row", marginBottom: 6, gap:2 ,fontSize:5 },
  ...
  tableHeaderCell: { fontSize: 8, fontWeight: "bold", padding: 4, borderRightWidth: 1, borderRightColor: "#e5e7eb" },
  tableRow: { flexDirection: "row", borderBottomWidth: 1, borderBottomColor: "#e5e7eb" },
  tableCell: { fontSize: 8, padding: 4, borderRightWidth: 1, borderRightColor: "#e5e7eb", textAlign: "center" },
  ...
  summaryRow: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 4, borderBottomWidth: 1, borderBottomColor: "#10b981" },
  summaryRowLast: { borderBottomWidth: 0 },
  footer: { marginTop: 15, paddingTop: 10, borderTopWidth: 1, borderTopColor: "#d1d5db", alignItems: "center" },
  ...
  return (
    <Document>
      {data.map((student) => {
        ...
        const totalDebits = statements.reduce((sum, s) => sum + parseFloat(s.debit || "0"), 0);
        const totalCredits = statements.reduce((sum, s) => sum + parseFloat(s.credit || "0"), 0);
        const currentBalance = parseFloat(statements[statements.length - 1]?.balance || "0");
        ...
        return (
          <Page key={student.id} size="A4" style={styles.page}>
            ...
            <View style={styles.header}>
              ...
              <Image style={styles.logo} src="/logo/university_logo.png" cache={false} />
              ...
              <Text style={styles.contactInfo}>Email: mawengcollege@gmail.com</Text>
            ...
            <View style={styles.studentInfoSection}>
              ...
              <View style={styles.infoColumn}>
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoValue}>{statements[0].semester}</Text>
                </View>
                 <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Programme:  </Text>
                  <Text style={styles.infoValue}>{student.programme}</Text>
            ...
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.tableHeaderCell, { flex: 0.8 }]}>Semester</Text>
                <Text style={[styles.tableHeaderCell, { flex: 0.8 }]}>Debit</Text>
                <Text style={[styles.tableHeaderCell, { flex: 0.8 }]}>Credit</Text>
                <Text style={[styles.tableHeaderCell, { flex: 0.8 }]}>Balance</Text>
              </View>

              {statements.map((s) => (
                <View key={s.reference} style={styles.tableRow}>
                  <Text style={[styles.tableCell, { flex: 0.8 }, styles.tableCellLeft]}>
                    {new Date(s.created_on).toLocaleDateString("en-GB")}
                  </Text>
                  <Text style={[styles.tableCell, { flex: 0.8 }, styles.tableCellLeft]}>
                    {s.statement_type}
                  </Text>
                  <Text style={[styles.tableCell, { flex: 2 }, styles.tableCellLeft]}>{s.reference}</Text>
                  <Text style={[styles.tableCell, { flex: 0.8 }, styles.tableCellLeft]}>{s.semester}</Text>
                  <Text style={[styles.tableCell, { flex: 0.8 }, styles.tableCellLeft]}>
            ...
            <View style={styles.summarySection}>
              ...
              <View>
                ...
                <View style={[styles.summaryRow, styles.summaryRowLast]}>
                  ...
                  <Text>{formatCurrency(currentBalance)}</Text>
                </View>
              </View>
            </View>
            ...
            <View style={styles.footer}>
              <Text style={styles.footerText}>This is an official fee statement of Maweng College.</Text>
              <Text style={styles.footerText}>For queries regarding this statement, please contact the bursar.</Text>
...
export const exportFeeStatementToPDF = async (data: FeeStatementStudent[]) => {
  ...
  link.href = url;
  link.download = `fee-statement-${studentName}-${admissionNumber}-${new Date().toISOString().split("T")[0]}.pdf`;
  link.click();
