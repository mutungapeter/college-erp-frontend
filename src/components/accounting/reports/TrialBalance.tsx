...
const TrialBalanceReports: React.FC = () => {
  ...
  const { filters, handleFilterChange } = useFilters({
    initialFilters: {
      as_of_date: searchParams.get("as_of_date") || new Date().toISOString().split('T')[0],
  ...
  const { data, isLoading, error } = useGetTrialBalanceQuery(queryParams, {refetchOnMountOrArgChange:true});
console.log("data", data)
  const TrialBalanceSection: React.FC<TrialBalanceSectionProps> = ({ 
    ...
    const hasAccounts = accounts && accounts.some(account => account.debit > 0 || account.credit > 0);
        ...
        <div className={`${getSectionBgColor()} px-6 py-4`}>
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-3">
              ...
              {hasAccounts && (
                <span className={`${getBadgeColor()} px-2 py-1 rounded-full text-xs font-medium`}>
                  {accounts.filter(account => account.debit > 0 || account.credit > 0).length} active
                </span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {accounts.map((account, index) => (
              <div key={index} className="grid grid-cols-5 gap-4 py-2 px-3 bg-white rounded border border-gray-100">
                <span className="text-sm text-gray-600 font-mono">{account.code}</span>
  ...
  const { total_debit: totalDebit, total_credit: totalCredit, balanced: isBalanced } = trialBalanceData?.totals || {
    total_debit: 0,
    total_credit: 0,
    balanced: false
  };

  const assetAccounts = trialBalanceData?.accounts?.filter(account => account.type === 'Asset') || [];
  const liabilityAccounts = trialBalanceData?.accounts?.filter(account => account.type === 'Liability') || [];
  const equityAccounts = trialBalanceData?.accounts?.filter(account => account.type === 'Equity') || [];
  const incomeAccounts = trialBalanceData?.accounts?.filter(account => account.type === 'Income') || [];
  const expenseAccounts = trialBalanceData?.accounts?.filter(account => account.type === 'Expense') || [];
    ...
    <div className="w-full font-nunito">
      ...
      <div className="mb-8 flex md:flex-row flex-col gap-4 md:gap-0 md:justify-between items-start">
        ...
        <div className="flex md:flex-row flex-col gap-2 md:items-center">
          <div>
            <label htmlFor="as_of_date" className="block text-sm mb-2 font-medium text-gray-700">As of Date</label>
            <input
              type="date"
              name="as_of_date"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="px-6 py-6 bg-white border-b border-gray-200">
          <h2 className="text-xl font-semibold mb-6 text-gray-900">Trial Balance Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className={`${isBalanced ? 'bg-green-50 border-green-200 hover:border-green-300' : 'bg-red-50 border-red-200 hover:border-red-300'} border rounded-lg p-4 hover:shadow-md transition-all duration-300`}>
              <div className={`text-sm mb-1 ${isBalanced ? 'text-green-700' : 'text-red-700'}`}>Balance Status</div>
              <div className={`text-xl font-semibold ${isBalanced ? 'text-green-800' : 'text-red-800'}`}>
        ...
        <div className={`px-6 py-6 border-t border-gray-200 ${isBalanced ? 'bg-green-50' : 'bg-red-50'}`}>
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-gray-800">Balance Verification</h3>
            <div className={`flex items-center space-x-2 px-4 py-2 rounded-lg ${isBalanced ? 'bg-green-100 border border-green-200' : 'bg-red-100 border border-red-200'}`}>
              {isBalanced ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div className="bg-white rounded-lg p-4 border border-blue-200 shadow-sm">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-lg text-blue-600">{formatCurrency(totalDebit)}</span>
              </div>
            </div>
            <div className="bg-white rounded-lg p-4 border border-red-200 shadow-sm">
              <div className="flex justify-between items-center">
                <span className="text-gray-700 font-medium">Total Credits</span>
                <span className="font-bold text-lg text-red-600">{formatCurrency(totalCredit)}</span>
              </div>
            </div>
          </div>
          ...
          {!isBalanced && (
            <div className="bg-red-100 border border-red-200 rounded-lg p-4 mb-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-red-900">{formatCurrency(Math.abs(totalDebit - totalCredit))}</span>
              </div>
            </div>
          )}
          
          <div className={`rounded-lg p-4 ${isBalanced ? 'bg-green-100 border border-green-200' : 'bg-red-100 border border-red-200'}`}>
            <div className={`flex items-center space-x-2 text-sm ${isBalanced ? 'text-green-800' : 'text-red-800'}`}>
              {isBalanced ? (
