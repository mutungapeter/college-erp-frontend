...
const BalanceSheetReports: React.FC = () => {
  ...
  const { data, isLoading, error } = useGetBalanceSheetQuery(queryParams, {refetchOnMountOrArgChange:true})
        ...
        <div className={`${getSectionBgColor()} px-6 py-4`}>
          ...
          <div className="space-y-2">
            {items.map((item, index) => (
              <div key={index} className="flex justify-between items-center py-2 px-3 bg-white rounded border border-gray-100">
                <span className="text-sm text-gray-700">{item.name}</span>
                <span className={`text-sm font-medium ${item.balance > 0 
                  ? getSectionColor()
                  : 'text-gray-400'
                }`}>
  ...
  const { Assets: totalAssets, "Liabilities + Equity": totalLiabilitiesEquity, Balanced: isBalanced } = balanceSheetData?.Totals || {
    Assets: 0,
    "Liabilities + Equity": 0,
    Balanced: false
  };

  const totalLiabilities = balanceSheetData?.Liabilities?.reduce((sum, item) => sum + item.balance, 0) || 0;
  const totalEquity = balanceSheetData?.Equity?.reduce((sum, item) => sum + item.balance, 0) || 0;
    ...
    <div className="w-full font-nunito">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="px-6 py-6 bg-white border-b border-gray-200">
          <h2 className="text-xl font-semibold mb-6 text-gray-900">Balance Sheet Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 hover:shadow-md hover:border-purple-300 transition-all duration-300">
              <div className="text-purple-700 text-sm mb-1">Liabilities + Equity</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                  <div className="bg-white rounded-lg p-4 border border-blue-200 shadow-sm">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-lg text-blue-600">{formatCurrency(totalAssets)}</span>
                    </div>
                  </div>
                  <div className="bg-white rounded-lg p-4 border border-purple-200 shadow-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-700 font-medium">Liabilities + Equity</span>
                      <span className="font-bold text-lg text-purple-600">{formatCurrency(totalLiabilitiesEquity)}</span>
                    </div>
                  </div>
                </div>
                ...
                {!isBalanced && (
                  <div className="bg-red-100 border border-red-200 rounded-lg p-4 mb-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-bold text-red-900">{formatCurrency(Math.abs(totalAssets - totalLiabilitiesEquity))}</span>
                    </div>
                  </div>
                )}
