...
console.log("queryParams",queryParams)
  ...
  const { data:booksData, isLoading, error, refetch } = useGetBorrowedBooksFinesQuery(queryParams, {
...
console.log("booksData",booksData)
  ...
  const columns: Column<FIneType>[] = [
   ...
   
    {
      ...
      cell: (item: FIneType) => (
        <span>
          <span className="text-xs font-normal">{item.fine_per_day}</span>
        </span>
      ),
    },
    {
      ...
      cell: (item: FIneType) => (
        <span>
          <span className="text-xs font-normal">{item.calculated_fine}</span>
        </span>
      ),
    },
...
<RequestFinePayment refetchData={refetch} data={item} />
    ...
    <>
      <div className="bg-white w-full  p-1 shadow-md rounded-lg font-nunito">
        <div className=" p-3  flex flex-col md:flex-row md:items-center lg:items-center md:gap-0 lg:gap-0 gap-4 lg:justify-between md:justify-between">
          <h2 className="font-semibold text-black text-xl">Issued Books Fines Records</h2>
...
export default BorrowedBooksFines;
