...
const HostelRoomOccupants = ({ room_id }: Props) => {
  ...
  const [removeHostelRoomOccupant, { isLoading: isRemoving }] =
    useRemoveHostelRoomOccupantMutation();
  ...
  const columns: Column<StudentDetailsType>[] = [
...

    {
      ...
      accessor: 'user',
      cell: (item: StudentDetailsType) => (
        <span className="text-sm font-nunito ">{item.user.phone_number}</span>
      ),
    },
    {
      header: 'Email',
    ...
    <>
      <div className="bg-white w-full  p-1 shadow-md rounded-lg font-nunito">
        <div className="flex flex-col gap-3 p-3">
          <Link
            href={`/dashboard/hostels/${occupantsData?.results?.room.hostel.id}`}
        ...
        <div className=" p-3  flex flex-col md:flex-row md:items-start lg:items-start md:gap-0 lg:gap-0 gap-4 lg:justify-between md:justify-between">
          <div className="flex flex-col gap-2">
            <h2 className="font-extrabold  text-lg pb-2">Occupants</h2>

            <h2 className="font-bold  text-sm">
              <span className=" font-normal text-sm">Room No</span>:{' '}
              {occupantsData?.results?.room.room_number || 'Room'}
            </h2>

            <small className=" rounded-md text-sm  font-bold ">
              <span className="  font-normal text-sm">Hostel:</span>{' '}
              {occupantsData?.results?.room.hostel.name || 'Missing hostel'}
            </small>
            <div className="text-sm  md:text-base md:font-bold">
              <span className=" font-normal text-sm">Room Capacity: </span>
              {occupantsData?.results?.room.room_capacity ?? '-'}
            </div>
            <div className="text-sm  md:text-base md:font-bold">
              <span className=" font-normal text-sm">Available Spots: </span>
              {occupantsData?.results?.available_spots ?? '-'}
            </div>
          </div>

          <div className="flex flex-col gap-2">
            <span
              className={`text-xs font-medium px-2 py-1 rounded-md ${occupantsData?.results.room?.status === 'Fully Occupied' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}
            >
        ...
        {isLoading ? (
          ...
          <DataTable
            data={
              occupantsData?.results?.occupants || occupantsData?.results || []
            }
            columns={columns}
            isLoading={isLoading}
            error={error}
