...
import { useUpdateApplicationMutation, useGetApplicationQuery } from "@/store/services/admissions/admissionsService";
...
const ApplicationDetails = ({ application_id }: Props) => {
  const { data, isLoading, isError, refetch } = useGetApplicationQuery(application_id);
  ...
  return (
    <section className="relative mx-auto w-full bg-white shadow-md rounded-lg overflow-hidden print:shadow-none print:rounded-none">
      <div className="px-4 md:px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b gap-3 print:bg-white">
        <Link
