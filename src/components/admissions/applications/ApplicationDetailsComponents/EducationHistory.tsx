...
const EducationHistoryCard = ({
  ...
  const educationHistory =
    applicationDetails.application_education_history || [];
  const [deleteEducationHistory, { isLoading: isDeleting }] =
            ...
            <tbody className="divide-y divide-gray-200">
              {educationHistory.map((education) => (
                <tr key={education.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span
                      className={`px-2 py-1 inline-flex text-xs leading-5 font-medium rounded-full ${getEducationLevelColor(education.level)}`}
                    >
                      {education.level}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center">
                      {education.graduated ? (
                        <>
                          <span className="text-green-800 bg-green-100 text-xs rounded-3xl py-1 px-2">
                            Graduated
                          </span>
                        </>
                      ) : (
                        <>
                          <span className="text-amber-800 bg-amber-100 text-xs rounded-3xl py-1 px-2">
                            Not Graduated
                          </span>
                        </>
                      )}
