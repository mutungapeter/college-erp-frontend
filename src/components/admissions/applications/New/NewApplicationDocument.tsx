...
import { ApplicationDocumentCreate, applicationDocumentCreateSchema } from "@/schemas/admissions/applicationDocuments";
...
const ApplicationDocumentUploadButton = ({ studentApplicationId, refetchData }: Props) => {
  ...
  const [createApplicationDocument, { isLoading: isUploading }] = useCreateApplicationDocumentMutation();
console.log("Student application ID", studentApplicationId)
  ...
  return (
    <>
      <div
        onClick={openModal}
        className="flex items-center space-x-2 md:p-2 p-2 w-fit bg-blue-600 hover:bg-blue-700 text-white rounded-md transition duration-300 shadow-sm cursor-pointer"
      >
